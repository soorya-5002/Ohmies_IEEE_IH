...
void loop() {
  ...
  gyro_signals();
  delay(500);
  if((abs(AnglePitch) > 35)||(flexPin <625)){
    tone(buzzerPin,15000,500);
    digitalWrite(ledPin, HIGH);
    Serial.println("VERY BAD POSTURE");
  }
  else if((abs(AnglePitch) > 20 && abs(AnglePitch) <= 35) ||  (625 <= flexPin && flexPin < 800 )){
      tone(buzzerPin,10000,500);
      digitalWrite(ledPin, HIGH);
      Serial.println("BAD POSTURE");
  }
  else if((abs(AnglePitch) > 10 && abs(AnglePitch) <= 20)|| (800 < flexPin && flexPin <= 1000)){
