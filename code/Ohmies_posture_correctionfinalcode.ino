...
void loop() {
  ...
  gyro_signals();
  delay(500);
  if((abs(AnglePitch) <= 50)||(flexPin>= 3562)){
    tone(buzzerPin,15000,500);
    digitalWrite(ledPin, HIGH);
    message="VERY BAD POSTURE";
    Serial.println(message);
  }
  else if((abs(AnglePitch)<=60 && abs(AnglePitch)>50) ||  (3413<= flexPin && flexPin < 3562 )){
      tone(buzzerPin,10000,500);
      digitalWrite(ledPin, HIGH);
      message="BAD POSTURE";
      Serial.println(message);
  }
  else if((abs(AnglePitch) > 60 && abs(AnglePitch) <= 70)|| (3175<= flexPin && flexPin < 3413)){
      tone(buzzerPin,5000,500);
      digitalWrite(ledPin, HIGH);
      message="Moderately BAD POSTURE";
      Serial.println(message);
  }
  else {
      noTone(buzzerPin);
